...
public class Wall extends MazeObject
{
	...
	{
		// TODO: Implement the algorithm listed above
		if((myPosY-UNIT > -1)&& board[(myPosY)/UNIT-1][myPosX/UNIT] == 2) {
			myColor = new Color(20, 5, 0);
		}
		
		if((myPosY+UNIT < 20*UNIT)&& board[myPosY/UNIT+1][myPosX/UNIT] == 2) {
			myColor = new Color(20, 6, 0);
		}
		
		if((myPosX+UNIT < 20*UNIT)&& board[myPosY/UNIT][(myPosX/UNIT)+1] == 2) {
			myColor = new Color(20, 3, 0);
		}
		
		if((myPosX-UNIT > -1)&& board[myPosY/UNIT][(myPosX/UNIT-1)] == 2) {
			myColor = new Color(20, 15, 0);
		}
		
		if(Player.getMethod().equals("You beat Rupal's Maze!")) {
			myColor = new Color(20, 25, 65);
		}
	}
